...
const SocilaCreate = (props) => {
    const router=useRouter();
    const [submit, setSubmit] = useState(false);
    let [data, setData] = useState({
        facebook: props.data[0].facebook,
        youtube: props.data[0].youtube,
        twitter: props.data[0].twitter,
        linkedin: props.data[0].linkedin
    });
    ...
    const formSubmit =async (e) => {
        ...
        if(IsEmpty(data.facebook)){
            ...
            let res=await fetch(`/api/dashboard/social?id=${props.id}`,options);

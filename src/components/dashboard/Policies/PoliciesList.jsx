...
const PoliciesList = (props) => {
    ...
    const PageKeyOnChange =(e) => {
        ...
        router.push(`/admin/dashboard/PoliciesList?pageNo=1&perPage=${(parseInt(e.target.value))}&searchKey=${searchKey}`);
    };
    const handlePageClick = ({ selected }) => {
        //setPageNo(1)
        router.push(`/admin/dashboard/PoliciesList?pageNo=${selected + 1}&perPage=${perPage}&searchKey=${searchKey}`);
        router.refresh()
    };
    const searchOnChange=(e)=>{
        ...
        if ((e.target.value.length) === 0) {
            setSearchKey("0")
            router.refresh();
            router.push(`/admin/dashboard/PoliciesList?pageNo=1&perPage=${perPage}&searchKey=0`);
            
        }
        else {
            router.refresh();
            router.push(`/admin/dashboard/PoliciesList?pageNo=1&perPage=${perPage}&searchKey=${e.target.value}`)
    ...
    const formSubmit = async () => {
        setSubmit(true);
        router.push('/admin/dashboard/PoliciesCreate')
     }
    const onDelete = async (id) => {
        ...
        if (Result.isConfirmed) {
            ...
            let res=await fetch(`/api/dashboard/policies?id=${id}`,options);
                ...
                <div className='row'>
                    ...
                    <div className='col-lg-12'>
                        ...
                        <button onClick={formSubmit} className='frm-btn'>
                            {submit ? <LoaderButton /> :"New Policies"}
                        </button>
                        </div>
                                    ...
                                    <div className='table-responsive data-table'>
                                            ...
                                            <thead className='sticky-top bg-white z-1'>
                                                <tr>
                                                    ...
                                                    <th className="text-uppercase text-xxs font-weight-bolder opacity-8">description</th>
                                                        ...
                                                        return <tr key={i}>
                                                                ...
                                                                <td>
                                                                    <div className='text-center'>
                                                                        <Link href={`/admin/dashboard/PoliciesUpdate?id=${item['id']}`}><AiOutlineEdit className='pen'/></Link>
